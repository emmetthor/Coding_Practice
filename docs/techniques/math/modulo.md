...
通常取餘不好做大量計算，因此有一些方式可以把取餘轉成不同數學模型再來計算。
...
由於 $10 \equiv 1 \pmod 3$，因此以十進位表示的數都可以改寫。

若有一數 $A = \sum_{i = 0}^{n} (10^i \times a_i)$，則：

$$
A \equiv \sum_{i = 0}^{n} (1^i \times a_i) \equiv \sum_{i = 0}^{n} a_i \pmod 3
$$

因此，透過這樣的轉換就可以只看**每位數的總和**來做取。
