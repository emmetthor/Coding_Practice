...
| 相鄰交換、抽取順序 | Swap Linked Technique | 把交換次數轉成逆序對或抽取成本 |
...
| 數字範圍 $[0,10^{10^6}]$ 且限制與各位數有關 | Digit DP | 常見狀態：位置、限制上界、前綴零、題目狀態 |
| 總分固定的雙人零和賽局 | 定和賽局／Negamax | 一方最大化等價另一方最小化 |
| 函數圖上的轉移且轉移次數 $\ge 10^9$ | 倍增 | 預處理 $2^j$ 步後到哪裡 |
...
| 恰好為 $k$ 的計數 | 用差分轉換成「$\le k$ 的個數減去 $<k$ 的個數」 |
