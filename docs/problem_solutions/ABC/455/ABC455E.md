...
4. 若子字串中的 $A_r - A_l = B_r - B_l = C_r - C_l$，其實可以轉換成：$(A_r - B_r, A_r - C_r) = (A_l - B_l, A_l - C_l)$ (請見 [數學式化簡](/Coding_Practice/techniques/math/equation_optimize))。
5. 而 $(A_r - B_r, A_r - C_r) = (A_l - B_l, A_l - C_l)$ 其實可以透過 [等價類計數](/Coding_Practice/techniques/math/counting_equivalent_states) 來計數，並貢獻答案為 $\binom{cnt}{2}$。
6. 因此，計算出所有需要計算的個數後，就可以輸出答案。

## 實作方法

[程式碼](https://github.com/emmetthor/Coding_Practice/blob/main/problems/ABC/455/E_Unbalanced_ABC_Substrings.cpp)
